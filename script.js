...
(() => {
  ...
  document.addEventListener("click", (e) => {
    if (mainNav.classList.contains("is-open") && !mainNav.contains(e.target) && e.target !== menuToggle && !menuToggle.contains(e.target)) {
      closeMenu();
    }
  });
  ...
  function renderCategories(tools) {
    ...
    categoryGrid.innerHTML = categories.map((cat) => `
      <a class="category-card" href="#tools" data-category="${escapeHtml(cat)}" role="listitem">
    ...
    // Clicking a category card filters the tools grid to that category
    // (same behaviour as the filter chips) and scrolls to it, instead of
  ...
  function renderTools(tools) {
    ...
    const base = activeCategory === "all"
      ? (tools.filter((t) => t.popular).length ? tools.filter((t) => t.popular) : tools)
      : tools;
    const filtered = activeCategory === "all" ? base : base.filter((t) => t.category === activeCategory);
