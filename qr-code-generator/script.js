/**
 * QR Code generator library — public-domain algorithm (Reed-Solomon based),
 * adapted from the well-known qrcode-generator implementation (Kazuhiko Arase, MIT).
...
/* ==========================================================================
   Multi Tools — QR Code Generator UI logic
   The `qrcode` object above is a self-contained QR encoding engine
   (Reed–Solomon error correction, mask pattern selection, etc.) implemented
   in vanilla JavaScript with no external dependencies or CDN requirement.
   A from-scratch QR encoder is unavoidable here because generating a valid,
   scannable QR code requires implementing the full QR spec (data encoding
   modes, error-correction polynomials, mask evaluation) — there is no way
   to draw a correct QR code with plain canvas drawing alone.
   ========================================================================== */
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var errorLevelSelect = document.getElementById('qrErrorLevel');
    ...
    // ---- Escape special characters for WiFi QR payload (RFC-ish MECARD/WiFi spec) ----
    function escapeWifiField(value) {
      return value.replace(/([\\;,:"])/g, '\\$1');
    }

    // ---- Build the raw string to encode, based on selected type ----
    function buildPayload() {
      var type = typeSelect.value;

      if (type === 'text') {
        var text = document.getElementById('textInput').value.trim();
        if (!text) { setError('Please enter some text.'); return null; }
        return text;
      }

      if (type === 'url') {
        var url = document.getElementById('urlInput').value.trim();
        if (!url || url === 'https://') { setError('Please enter a website URL.'); return null; }
        if (!/^https?:\/\//i.test(url)) {
          url = 'https://' + url;
        }
        return url;
      ...
      if (type === 'email') {
        var address = document.getElementById('emailAddress').value.trim();
        if (!address) { setError('Please enter an email address.'); return null; }
      ...
      if (type === 'phone') {
        ...
        if (!phone) { setError('Please enter a phone number.'); return null; }
        return 'tel:' + phone;
      }

      if (type === 'sms') {
        var smsNumber = document.getElementById('smsNumber').value.trim();
        if (!smsNumber) { setError('Please enter a phone number.'); return null; }
        var smsMessage = document.getElementById('smsMessage').value.trim();
      ...
      if (type === 'wifi') {
        ...
        var hidden = document.getElementById('wifiHidden').checked;
        var wifiString = 'WIFI:T:' + encryption + ';S:' + escapeWifiField(ssid) + ';';
    ...
    function renderQr(qr, size, fgColor, bgColor) {
      var moduleCount = qr.getModuleCount();
      var quietZone = 4; // modules of margin, per QR spec recommendation
      ...
      for (var row = 0; row < moduleCount; row += 1) {
        for (var col = 0; col < moduleCount; col += 1) {
          if (qr.isDark(row, col)) {
            var x = (col + quietZone) * cellSize;
            var y = (row + quietZone) * cellSize;
            // Slight overlap (0.5px) avoids sub-pixel seams between modules.
    ...
    function generate() {
      ...
      var errorLevel = errorLevelSelect.value;
      var fgColor = fgColorInput.value;
      var bgColor = bgColorInput.value;

      try {
        var qr = qrcode(0, errorLevel);
        qr.addData(payload);
        qr.make();
        renderQr(qr, size, fgColor, bgColor);

        canvas.style.display = 'block';
        placeholder.style.display = 'none';
        downloadBtn.disabled = false;
        lastFilename = 'qr-code-' + typeSelect.value;
      } catch (err) {
        setError('Could not generate a QR code for this content. Try shortening it.');
    ...
    function downloadQr() {
      ...
      link.download = lastFilename + '.png';
    ...
    // Allow Enter key inside single-line inputs to trigger generation
    document.querySelectorAll('.tool-column input').forEach(function (el) {
